		...
		<form method="post">
			<table summary="상세보기">
				...
				<tbody>
					...
					<tr>
						<th>내용:</th>
						<td><textarea cols="50" rows="8">${bvo.content }</textarea></td>
					</tr>
					<tr>
						<th>첨부파일:</th>
						<c:choose>
							...
							<c:otherwise>
								<td>
									<a href="bbs_down.do?f_name=${bvo.f_name}"> 
	...
	<div style="padding: 50px; width: 580px; margin: 0 auto;">
		<form method="post">
			<fieldset>
				...
				<input type="hidden" name="b_idx" value="${bvo.b_idx}">
				<input type="button" value="댓글저장" onclick="comm_write(this.form)" /> 
			</fieldset>
		</form>
	</div>
	<%-- 댓글 출력 부분 --%>
		...
		<c:forEach var="k" items="${comm_list}">
			<div style="border: 1px solid #cc00cc; width: 400px; margin: 20px; padding: 20px;" >
				<form method="post">
					...
					<input type="button" value="댓글삭제" onclick="comm_delete(this.form)" />
