...
<script type="text/javascript">
	function board_write() {
		location.href = "board_write.do?cPage=${paging.nowPage}";
	}
</script>
</head>
<body>
			...
			<tbody>
				<c:choose>
					<c:when test="${empty board_list}">
						<tr><td colspan="5"><h3>게시물이 없습니다.</h3></td></tr>
					</c:when>
					<c:otherwise>
						<c:forEach var="k" items="${board_list}" varStatus="vs">
							<tr>
								<td>
									${ paging.totalRecord - 
										( (paging.nowPage -1) * paging.numPerPage + vs.index ) }
								</td>
								<td style="text-align: left;">
									...
									<c:choose>
										...
										<c:otherwise>
											<a href="board_detail.do?bo_idx=${k.bo_idx}&cPage=${paging.nowPage}">
											${k.title}</a>
										</c:otherwise>
									</c:choose>
								</td>
